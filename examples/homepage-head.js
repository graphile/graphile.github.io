...
function PersonTypePlugin(builder) {
  builder.hook('init', (_, {
    addType,
    graphql: { GraphQLObjectType, GraphQLString, GraphQLEnumType, GraphQLNonNull },
    newWithHooks,
  }) => {
    const Color = newWithHooks(GraphQLEnumType, {
      name: 'Color',
      values: {
        RED: { value: 'red' },
        GREEN: { value: 'green' },
    ...
    const PersonType = newWithHooks(GraphQLObjectType, {
      name: 'Person',
      fields: {
        firstName: {
          type: GraphQLString,
  ...
  builder.hook('GraphQLObjectType:fields', (fields, { extend, getTypeByName, graphql: { GraphQLNonNull } }, { scope: { isRootQuery } }) => {
    if (!isRootQuery) {
      return fields;
    }
...
(async () => {
  ...
  const updatedSchema = await buildSchema([...plugins, CommentToDeprecationPlugin]);
