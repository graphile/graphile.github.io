...
`makeWrapResolversPlugin` is a helper function for you to easily generate your own custom plugin (as a JS module) for 'wrapping' the resolvers generated by PostGraphile. **If using PostGraphile CLI, the generated plugin module should then be provided as an argument to `--append-plugins`, or included in the `appendPlugins` array option when using PostGraphile as a library.**
...
As mentioned above, the generated plugin module should then be provided as an argument to `--append-plugins` when using PostGraphile CLI, or included in the `appendPlugins` array option when using PostGraphile as a library.

The above examples defined single plugin object generated by calling `makeWrapResolversPlugin` and exported as the only element in a Javascript file (module). This is equivalent to the following in ES6:
