...
system which is plugin-based.

Despite this huge change, v4 is still broadly compatible with v3; you can
read more about migrating from v3 to v4 in our [migration
guide](/postgraphile/v3-migration/).
...
the posts have received. It uses a computed column for the user `fullName`s,
...
in `condition` argument) without having to have users maintain forks of the
...
PostGraphile no longer includes resources from extensions automatically, so
your schema by default only includes the functionality you've added yourself.
(Go back to the old behaviour with `--include-extension-resources`.)
...
comments](/postgraphile/smart-comments/) feature. This feature is highly
flexible, so get in touch if you can think of more ways for it to help you
build your perfect schema!
...
You no longer have to trust us to come up with the best names for your
fields. You can override individual fields using our [smart
comments](/postgraphile/smart-comments/) feature, or override the names that
we auto-generate by using a plugin to [override our
inflectors](/postgraphile/inflection/) with your own.
...
which changes fields like `userByAuthorId: User` into simply `author: User` -
...
As part of the performance work, we now select only the fields we need (and
...
and use the one-to-one relationship feature to link them.
